...
<template>
  <div class="card bg-base-100 shadow-xl image-full text-left">
    ...
    <div class="card-body">
      ...
      <p>{{ clip.title }}</p>
      <p> {{ clip.duration }}s {{ clip.view_count }} {{ clip.created_at }} {{ clip.published }} {{ clip.language }}</p>
      ...
      <div class="card-actions">
        ...
        <button v-if="clip.embed_url != null" @click="() => { isPlayerHidden = !(isPlayerHidden) }"
          ...
          <a role="button" target="_blank" :href="clip.url" class="btn btn-info"> <i
              class="fa-regular fa-window-restore"></i> Link </a>
        </div>
      </div>
        ...
        <div v-else class="justify-begin">
          <button @click="$emit('up', clip.url)" class="btn btn-primary"> Up </button>
          <button @click="$emit('down', clip.url)" class="btn btn-primary"> Down </button>
          <button @click="$emit('top', clip.url)" class="btn btn-success"> Top </button>
          <button @click="$emit('bottom', clip.url)" class="btn btn-success"> Bottom </button>
          <button @click="$emit('remove', clip.url)" class="btn btn-error"> <i class="fa-regular fa-trash-can"></i>
            Remove </button>
        </div>
